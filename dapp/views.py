...
def home(request):
    if request.method=="POST":
        ...
        # print('prodcut name received: ',name)
        amount=int(request.POST.get("amount1")) 
        email=request.POST.get('email')
        # print("name:",name)
        # print("amount:",amount)
        # creating client and verifying the funct 
        client=razorpay.Client(auth=("rzp_test_cPaYFJ7TbiBvUh","NlNaz6IDiqQ0lnItdjYJ1LHd"))
...
def success(request):
    if request.method =="POST":
        ...
        # print(check) If the value is none,thwn payment is completed and accepted successfully by razorpay and respective bank.
